...
const ModuleHero: React.FC<ModuleHeroProps> = ({
  ...
  const parsedTitle = title ? asText(title) : "[LEARNING_MODULE]";
      ...
      title={
        <React.Fragment>
          ...
          {body && (
            <React.Fragment>
              <MobileOnly>
                <Paragraph size={"medium"}>{asText(body)}</Paragraph>
              </MobileOnly>
              <TabletUp>
                <Paragraph size={"large"}>{asText(body)}</Paragraph>
              </TabletUp>
            </React.Fragment>
          )}
        </React.Fragment>
                ...
                <Box direction={"row"} justify={"center"}>
                  {guideDownload && hasAttachedMedia(guideDownload) && (
                    <Button
                      ...
                      link={guideDownload}
                      icon={
                        <DownloadIcon url={getLinkUrl(guideDownload) ?? ""} />
                      }
                    />
                  )}
                  {guideLink && (
